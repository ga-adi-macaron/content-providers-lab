...
public class MyDBHandler extends SQLiteOpenHelper {
    ...
    public Cursor getStocks(String selection, String[] selectionArgs, String orderBy, String id) {
        SQLiteDatabase db = getReadableDatabase();
        Cursor cursor;

        if(id == null) {
            cursor = db.query(TABLE_STOCKS, null, selection, selectionArgs, null, null, orderBy);
        } else {
            cursor = db.query(TABLE_STOCKS, null, COLUMN_ID+" = ?", new String[]{id}, null, null, orderBy);
        }
        db.close();
        return cursor;
    }

    public int deleteStockById(String selection, String[] args, String id) {
        SQLiteDatabase db = getWritableDatabase();
        int rowsDeleted;
        if(id != null) {
            rowsDeleted = db.delete(TABLE_STOCKS, COLUMN_ID + "=?", new String[]{id});
        } else {
            rowsDeleted = db.delete(TABLE_STOCKS, selection, args);
        }
        db.close();
        return rowsDeleted;
    }
    ...
    public int updateStock(ContentValues values, String selection, String[] selectionArgs, String id) {
        ...
        if(id == null) {
            rowsUpdated = db.update(TABLE_STOCKS, values, selection, selectionArgs);
        } else {
            rowsUpdated = db.update(TABLE_STOCKS, values, COLUMN_ID+" = ?", new String[]{id});
        }
